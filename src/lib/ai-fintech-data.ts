...
export interface AIFintechService {
  id: string;
  icon: LucideIcon;
  title: string;
...
export const aiFintechServices: AIFintechService[] = [
  {
    id: "ai-text-chatbot",
    icon: MessageSquare,
...
export function getAIFintechServiceById(id: string): AIFintechService | undefined {
  return aiFintechServices.find(service => service.id === id);
}
